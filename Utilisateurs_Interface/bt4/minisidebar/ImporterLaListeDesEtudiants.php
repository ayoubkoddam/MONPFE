<?php
    session_start();
    error_reporting(0);
    require_once '../../../readExcel/classes/PHPExcel/IOFactory.php';
if (isset($_POST['Importer'])) {
   if(isset($_FILES['excelFile']) && !empty($_FILES['excelFile']['tmp_name'])){
        ...
        $conn = new mysqli("localhost","root","","monpfe");


        $sql= '';
        for($row=1; $row <= count($getSheet); $row++){
            $xx = "'"  . implode("','", $getSheet[$row]) . "'";
            $sql = "INSERT INTO etudiant (CNE,Cycle,Option_S6,Nom_filiere)VALUES($xx);";
            if($conn->query($sql)===TRUE){
                $message='<h4 style="color:green; text-align:center;"><i class="ti-check">'.' la liste des étudiants a été importé avec succès</i></h4>';
            }
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="card card-body">
                            <h4 class="card-title">Importation de la liste des étudiants dans la base de données</h4><br><br>
                        <form method="post" action="" enctype="multipart/form-data">
                            <div class="input-group">
                              <div class="custom-file">
                                <input type="file" class="custom-file-input" name="excelFile" id="inputGroupFile04" aria-describedby="inputGroupFileAddon04">
                                <label class="custom-file-label form-control" for="inputGroupFile04">Sélectionner le fichier</label>
                              </div>
                              <div class="input-group-append">
                                    <input type="submit" name="Importer"class="btn btn-outline-secondary"value="Importer">
                              </div>
                            </div>
                        </form><br><br>
